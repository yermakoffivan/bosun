...
fn build_rows() -> Vec<Row> {
    ...
    vec![
        ...
        Binding(
            "m",
            "Modify session (path, agent, flags) — applies on next Ctrl+R",
        ),
        Binding(
            "D",
        ...
        Binding(
            "a (in D prompt)",
            "Kill every tab in a multi-tab container at once",
        ),
        Binding("E", "Open session's path in configured editor"),
        Binding(
            "Ctrl+L",
            "Redraw + refresh — recover from a terminal reset (e.g. Cmd+R)",
        ),
        Binding(
            "Ctrl+B",
